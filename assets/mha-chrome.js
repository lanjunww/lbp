...
(function(){
  ...
  function createKoalaController(){
    ...
    function msg(h,s){
      ...
      const move = (planned && planned.t) ? planned.t : clusterActivity();
      const hour = Math.max(1, Math.min(5, parseInt(h || 1, 10)));
      const hourText = `${h} hour${h>1?'s':''}`;
      const sets = {
        low: [
          {title:'Koala tiny nudge 🌿', body:`${hourText} sitting. Your back may like a tiny reset: ${move}. Keep it light and easy.`},
          {title:'Koala stretch snack 🐨', body:`Two-hour sit streak spotted. Try ${move} — a small move now keeps the day flowing.`},
          {title:'Koala says: unlock the hips 🔓', body:`${hourText} sitting. Your back is not angry, just bored. Stand, breathe, and try ${move}.`},
          {title:'Koala mini mission ✨', body:`${hourText} sitting. No big workout needed — ${move} is enough to keep the habit alive.`},
          {title:'Koala rescue wobble 🚶', body:`${hourText} sitting. Time for a friendly reset: ${move}. Future-you may quietly thank you.`}
        ],
        medium: [
          {title:'Koala is side-eyeing the chair 👀', body:`${hourText} sitting. Your plan has a gentle option ready: ${move}. Start small, no perfection needed.`},
          {title:'Stone-mode alert 🗿', body:`Two hours sitting. Koala detected statue mode. Try ${move} to wake your back up.`},
          {title:'Koala says: shift the load ⚖️', body:`${hourText} sitting. Change position and try ${move}. Little breaks can reduce that steady low-back load.`},
          {title:'Back-friendly quest time 🎯', body:`${hourText} sitting. Pick a comfortable version of ${move}. Slow and repeatable beats intense and rare.`},
          {title:'Koala would like a plot twist 🐨', body:`${hourText} sitting. Let’s interrupt the chair story with ${move}. Your back gets a new scene.`}
        ],
        high: [
          {title:'Gentle Koala check-in 🌿', body:`${hourText} sitting. Try a very gentle version of ${move}. Move slowly and stop if symptoms feel worse.`},
          {title:'High-risk soft reset 🐨', body:`Two hours sitting. Your back deserves a careful break: ${move}. Comfortable range only.`},
          {title:'Koala says: protect the back shield 🛡️', body:`${hourText} sitting. Keep this tiny and safe: ${move}. No pushing through sharp pain.`},
          {title:'Slow-mode nudge 🐌', body:`${hourText} sitting. A small change of position helps. Try ${move}, gently, and keep breathing easy.`},
          {title:'Koala care reminder 💙', body:`${hourText} sitting. Choose the easiest version of ${move}. If pain, numbness or weakness increases, pause and seek advice.`}
        ]
      };
      const arr = sets[risk] || sets.medium;
      return arr[hour-1] || arr[arr.length-1];
    }
