...
V62 changes:
- Koala sitting notifications are now richer and more playful.
- Notification title/body vary by sitting hour and by risk level (low / medium / high).
- Messages still pull in the user’s planned activity or cluster-based suggestion.

V63 changes:
- risk-tool result page now shows Low/Medium/High risk info buttons with short educational tooltips.
- risk-tool result page now includes official external learn-more links (healthdirect, MHA, Quitline when smoking is flagged).
- Added a collapsible green smoking-support module beneath Mental Health Priority on the risk result page, Weekly Plan and Risk Insights whenever Q1 daily smoking is Yes.

V68 changes:
- Replaced risk-factor info icon links with clearer user-selected public education pages.
- Sedentary Time -> Harvard Health moving more / sitting less.
- Body Mass Index -> Cleveland Clinic lower back pain.
- Smoking Status -> Cleveland Clinic smoking and chronic pain.
- Physical Activity -> Johns Hopkins lower back pain lifestyle factors.
